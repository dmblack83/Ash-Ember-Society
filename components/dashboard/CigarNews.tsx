...
function LikeButton({
  ...
  useEffect(() => {
    ...
    // Load count + own like status in parallel
    Promise.all([
      supabase
  ...
  return (
    ...
    >
      <svg
        width="20" height="20" viewBox="0 0 20 20" fill={liked ? "var(--gold)" : "none"}
        aria-hidden="true"
      ...
      <div className="flex items-center gap-4 px-5 py-2">
        ...
        <button
          ...
          style={{
            ...
            padding: "8px 4px", touchAction: "manipulation",
            WebkitTapHighlightColor: "transparent",
          } as React.CSSProperties}
...
/* ------------------------------------------------------------------
   ...
   Desktop : centered modal 600px × 85vh, rounded all sides
   ------------------------------------------------------------------ */
    ...
    async function load() {
      try {
        ...
        const [{ data: { user } }, postsRes] = await Promise.all([
          ...
          supabase
            .from("blog_posts")
            .select("id, type, title, cover_image_url, excerpt, body, synopsis, source_name, source_url, published_at")
        ...
        if (user) {
          ...
          const { data: profile } = await supabase
            ...
            .eq("id", user.id)
            .single();
          setUserName(profile?.display_name ?? "Member");
        }
      } catch {
        setErrored(true);
      } finally {
