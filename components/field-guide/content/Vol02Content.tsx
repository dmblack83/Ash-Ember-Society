...
const PRIMINGS: { name: string; where: string; desc: ReactNode }[] = [
  {
    name:  "Medio Tiempo",
    where: "the crown",
...
const LANDS: { name: string; region: string; desc: ReactNode }[] = [
  {
    name:   "Cuba",
  ...
  return (
    <>
      ...
      >
        <thead>
          <tr>
            {["Priming", "Character & role in the blend"].map((h) => (
              <th
                key={h}
                style={{
                  textAlign:     "left",
        ...
        <tbody>
          {PRIMINGS.map((p, i) => (
            <tr key={p.name}>
              ...
              >
                {p.name}
                <span style={{ display: "block", fontFamily: S.serif, fontStyle: "italic", fontWeight: 400, fontSize: 12, letterSpacing: 0, textTransform: "none", color: S.gold, marginTop: 3 }}>
                  {p.where}
                </span>
              </td>
              <td
        ...
        {[
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <div style={{ fontFamily: S.serif, fontStyle: "italic", color: S.gold, fontSize: 14 }}>{role.num}</div>
              <h3 style={{ fontFamily: S.serif, fontWeight: 600, fontSize: 28, margin: 0, color: S.fg1, letterSpacing: "0.01em" }}>{role.name}</h3>
              <div style={{ fontFamily: S.serif, fontStyle: "italic", color: S.gold, fontSize: 14 }}>{role.gloss}</div>
            </div>
            <p style={{ fontFamily: S.sans, fontSize: 15, lineHeight: 1.7, color: "var(--muted-foreground)", margin: 0, maxWidth: "60ch" }}>{role.desc}</p>
          </div>
        ))}
      </div>
      ...
      >
        {LANDS.map((land) => (
          ...
          >
            ...
            <p style={{ fontFamily: S.sans, fontSize: 14, lineHeight: 1.6, color: "var(--muted-foreground)", margin: 0 }}>
              {land.desc}
            </p>
          </div>
        ))}
      ...
      {/* Colors panel */}
      <div style={{ margin: "28px 0 8px", background: "var(--card)", border: "1px solid rgba(212,160,74,0.22)", borderRadius: 6, padding: "28px 24px" }}>
        <div style={{ fontFamily: S.sans, fontSize: 10.5, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--muted-foreground)", textAlign: "center", marginBottom: 4 }}>
          Wrapper Color Scale
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", border: "1px solid rgba(212,160,74,0.18)" }}>
          {COLORS.map((c, i) => (
            <div
              key={c.name}
              style={{ padding: "14px 10px", borderRight: i < COLORS.length - 1 ? "1px solid rgba(0,0,0,0.25)" : "none", textAlign: "center" }}
            >
              <div style={{ height: 56, marginBottom: 10, border: "1px solid rgba(0,0,0,0.3)", background: c.cls }} />
              <div style={{ fontFamily: S.serif, fontWeight: 600, fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: S.fg1 }}>{c.name}</div>
