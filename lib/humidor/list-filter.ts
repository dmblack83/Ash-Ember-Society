...
export function matchesQuery(
  item: { cigar: { brand: string | null; series: string | null; name?: string | null; wrapper: string | null } },
  query: string,
): boolean {
  const q = query.trim().toLowerCase();
  if (!q) return true;
  return [item.cigar.brand, item.cigar.series, item.cigar.name, item.cigar.wrapper]
