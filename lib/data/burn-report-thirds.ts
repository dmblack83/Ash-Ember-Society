...
/* ------------------------------------------------------------------
   Batched per-third tag resolution — moved to a client-safe module
   (lib/data/burn-report-thirds-batch.ts) because this file imports
   the server Supabase client (next/headers). Re-exported here so
   existing server call sites keep their import path.
   ------------------------------------------------------------------ */

export {
  getBurnReportThirdsTaggedBatch,
  type BurnReportThirdTaggedRow,
} from "@/lib/data/burn-report-thirds-batch";
