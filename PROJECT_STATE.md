...
- **Runtime verification + prod diagnosis (2026-07-07)** — Before claiming any user-facing change works: run the `verify-in-app` project skill (`scripts/verify-in-app.mjs`, logged-in screenshots + console/5xx checks; needs the test-account creds from Dave). For any prod symptom: read `docs/runbooks/prod-diagnosis.md` FIRST — it maps symptoms to the evidence surfaces (marks, log markers, diagnostic tables, REST probes) and known root causes before any hypothesis.
- **Bottom-nav routes must stay static shells (RULE, 2026-07-07)** — `/home`, `/humidor`, `/lounge`, `/discover/cigar-news`, `/account` must prerender static (`○` in build output): no `getServerUser()` or server data fetch in their `page.tsx`; auth gates client-side via `useGatedSession`; data arrives via SWR. The bottom nav prefetches with `prefetch={true}`, and for a DYNAMIC route that runs the full server render on every app open — when #555 made `/lounge` (then dynamic) heavy, cold-load tab taps went dead until the connection warmed (fixed in #557). Corollary: any change that increases a route's server cost must check every `<Link>` targeting it for prefetch mode; new surfaces default to the shell pattern (reference: `HumidorRoute`, `LoungeRoute`), with dynamic rendering reserved for on-demand deep routes (detail pages) that have `loading.tsx`. Mechanical gate: `npm run check:shells` after a build (wired into the CI bundle-size job); update `scripts/check-static-shells.mjs` when nav tabs change.
